...
class ReportController extends Controller { public function index(): Response { return Inertia::render('Admin/Reports',['summary'=>['users'=>User::count(),'menus'=>Menu::count(),'orders'=>Order::count(),'reservations'=>Reservation::count(),'payments'=>Payment::count(),'revenue'=>Payment::where('status','verified')->sum('amount'),'pending_payments'=>Payment::where('status','pending')->count()],'orders'=>Order::with('user')->latest()->take(8)->get(),'reservations'=>Reservation::with(['user','table'])->latest()->take(8)->get()]); } public function statistics(): Response { return $this->index(); }
